...
Original file is located at
    https://colab.research.google.com/drive/1tTfcet5wj71byK6ichLPEhxazbgm1O-_
...
1. Concatenate Dataset in order to break the train - test sets in 60% - 40%.
"""

X_full = tf.concat([X, X_test], axis=0)
y_full = tf.concat([y, y_test], axis=0)

"""2. Select a subset of 10000 images from Cifar10 dataset and split it into 60%-40%"""
...
"""3. Split the test se into validation & test set (20% - 20%)"""
...
n_components_plot = 500
cumulative_variance = np.cumsum(pca.explained_variance_ratio_)[:n_components_plot]
...
Train SVM with different Kernels and default parameters in order to find the best one.
"""
...
con_matrix = confusion_matrix(y_test.ravel(), y_pred)
...
svm_best = SVC(kernel='poly', C=best_C)
svm_best.fit(X_train, y_train.ravel())
y_pred = svm_best.predict(X_test)
...
param_grid1 = {'C': [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.05, 0.1, 1],
...
accuracy_svm = accuracy_score(y_val.ravel(), y_pred)
...
con_matrix = confusion_matrix(y_test.ravel(), y_pred_test)
plt.figure(figsize=(10, 8))
sns.heatmap(con_matrix, annot=True, fmt='d', cmap='Blues', cbar=False,
            xticklabels=[str(i) for i in range(10)],
            yticklabels=[str(i) for i in range(10)])
...
print("Classification Report:\n", classification_report(y_test.ravel(), y_pred_test))

"""Show Correct and Incorrect Classification Results"""

svm_results = y_pred[:20].ravel()
...
for kernel in kernels:
    ...
    for c in C_values:
        ...
        svm.fit(X_train_lda, y_train.ravel())
        ...
        # validation set accuracy
        val_accuracy = accuracy_score(y_val.ravel(), y_pred_val)
        ...
        # calculate metrics
        kappa = cohen_kappa_score(y_test.ravel(), y_test_pred)
...
grid_search_linear.fit(X_train, y_train.ravel())
...
# Tets the model to the validation set
...
confusion_matrix_1 = confusion_matrix(y_test.ravel(), y_pred_test)
plt.figure(figsize=(10, 8))
...
# Confusion Matrix (Heatmap)
confusion_matrix_2 = confusion_matrix(y_test.ravel(), y_pred_test)
plt.figure(figsize=(10, 8))
...
"""POLYNOMIAL KPCA - Cross 5 Fold - Validation Grid Search"""
...
"""Linear SVC with rbf kpca, c=0.1
...
# kpca
kpca = KernelPCA(n_components_95, kernel='rbf', gamma=0.0005)
X_train_kpca = kpca.fit_transform(X_train)
X_val_kpca = kpca.transform(X_val)
X_test_kpca = kpca.transform(X_test)
...
svm_model.fit(X_train_lda, y_train.ravel())

prediciton = svm_model.predict(X_test_lda)
accuracy = accuracy_score(y_test.ravel(), prediciton)
...
pipeline = Pipeline([
    ('kpca', KernelPCA(kernel='rbf', n_components=n_components_95, gamma=0.0005)),
    ('lda', LDA(n_components=9)),
...
train_sizes_acc, train_scores_acc, val_scores_acc = learning_curve(pipeline, X_train, y_train.ravel(), cv=5, scoring='accuracy', train_sizes=train_sizes)

# loss
train_sizes_loss, train_scores_loss, val_scores_loss = learning_curve(pipeline, X_train, y_train.ravel(), cv=5, scoring='neg_log_loss', train_sizes=train_sizes)

# calculate means and std for accuracy
...
# calculate means and std for loss
...
# Plot Accuracy
plt.subplot(1, 2, 1)
plt.plot(train_sizes_acc, train_mean_acc, label='Training Accuracy', color='blue')
plt.plot(train_sizes_acc, val_mean_acc, label='Validation Accuracy', color='red')
plt.fill_between(train_sizes_acc, train_mean_acc - train_std_acc, train_mean_acc + train_std_acc, alpha=0.1, color='blue')
plt.fill_between(train_sizes_acc, val_mean_acc - val_std_acc, val_mean_acc + val_std_acc, alpha=0.1, color='red')
...
# Plot Loss
plt.subplot(1, 2, 2)
plt.plot(train_sizes_loss, train_mean_loss, label='Training Loss', color='blue')
plt.plot(train_sizes_loss, val_mean_loss, label='Validation Loss', color='red')
plt.fill_between(train_sizes_loss, train_mean_loss - train_std_loss, train_mean_loss + train_std_loss, alpha=0.1, color='blue')
plt.fill_between(train_sizes_loss, val_mean_loss - val_std_loss, val_mean_loss + val_std_acc, alpha=0.1, color='red')
plt.xlabel('Training Examples')
plt.ylabel('Loss')
