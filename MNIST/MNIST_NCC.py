...
Original file is located at
    https://colab.research.google.com/drive/10Dzo3Kp_lRhXeznnCCkXmLBXML1XfjtO
"""
...
X_train = pca_mnist.fit_transform(X_train)
X_val = pca_mnist.transform(X_val)
X_test = pca_mnist.transform(X_test)
...
grid_search = GridSearchCV(ncc_model, param_grid, cv=10, n_jobs=-1)
...
ncc_accuracy = accuracy_score(y_test, ncc_pred)
print("NCC Test set Accuracy:", ncc_accuracy)

ncc_results = ncc_pred[:20]
...
for kernel in kernels:
    ...
    grid_search = GridSearchCV(ncc, param_grid, cv=5, n_jobs=-1)
...
kpca = KernelPCA(n_components_95, kernel='poly', degree=6, coef0=10)
X_train_kpca = kpca.fit_transform(X_train)
X_test_kpca = kpca.transform(X_test)
